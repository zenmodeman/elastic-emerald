...
static inline bool32 IsIlluminatingMove(enum Move moveId)
{
    return gMovesInfo[SanitizeMoveId(moveId)].illuminatingMove;
}

static inline bool32 IsEnticingMove(enum Move moveId)
{
    return gMovesInfo[SanitizeMoveId(moveId)].enticingMove;
}

static inline bool32 IsGiftingMove(enum Move moveId)
{
    return gMovesInfo[SanitizeMoveId(moveId)].giftingMove;
}

static inline bool32 IsItemInteractingMove(enum Move moveId)
{
...
{
    #if TESTING
    if (moveId == MOVE_TOXIC && GetConfig(TOXIC_NEVER_MISS) < GEN_6)
       return FALSE;
    #endif
...
{
    #if TESTING
    if (moveId == MOVE_SHEER_COLD && GetConfig(SHEER_COLD_IMMUNITY) < GEN_7)
       return FALSE;
    #endif
