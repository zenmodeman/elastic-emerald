...
- **Uncommitted AI changes covered by this document:** A narrow `AI_FLAG_HEAVY_SWITCHING` fast-KO preservation rule replaces the stale broad bad-odds implementation reintroduced by the upstream switch-file refactor. Regular smart switching receives only the defensive-drop exception described below. The current uncommitted Metal Rush runtime repair makes its Defense rider honor ordinary stat-loss prevention, matching the existing `CanLowerStat` model; it does not change AI scoring or information policy. The Expansion 1.15.0 merge resolution retains the local AI cores and imperfect-information policy while adapting their runtime dependencies—especially ability-block prediction, Follow Me targeting, form changes, and generational configuration—to the upstream APIs.
