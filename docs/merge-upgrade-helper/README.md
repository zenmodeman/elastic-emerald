...
- Latest commit whose applicable project changes have been reviewed for this dossier: `3cbf01e171` (`First batch of commit history tests and feature restorations: March-August 2026`).
- Applicable work after that boundary: the uncommitted historical coverage audit described below.
- **Backward audit complete (2026-09-07):** every zenmodeman-authored commit from the former resume point `1b0f6dfc88` (`Woods items and Bug Catcher Lyle`, 2024-12-06) back through the first project change `f5a841eef6` (`Test ReadME commit`, 2023-12-31) has been reviewed. There is no older backward-pass commit remaining. The pass compared each surviving feature with current runtime/data/script anchors and existing `Zenmodeman:` coverage; merge/build-only commits and historical balance values superseded by later zenmodeman commits were not frozen into tests. Newer commits made after this historical audit began remain the next, forward-history scope.
