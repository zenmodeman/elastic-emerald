...
static bool32 AI_IsSemiInvulnerableAgainstMove(u32 battlerDef, u32 move);
...
static bool32 AI_IsDoubleSpreadMove(u32 battlerAtk, u32 move)
{
    ...
    for (u32 battlerDef = 0; battlerDef < MAX_BATTLERS_COUNT; battlerDef++)
    {
        ...
        if (IsBattlerAlive(battlerDef) && !AI_IsSemiInvulnerableAgainstMove(battlerDef, move))
            numOfTargets++;
    }
...
void RecordMovesBasedOnStab(u32 battler)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (move != MOVE_NONE
         ...
         && IS_BATTLER_OF_TYPE(battler, GetMoveType(move)))
            RecordKnownMove(battler, move);
    }
}
...
void RecordStatusMoves(u32 battler)
{
    ...
    {
        u32 move = gBattleMons[battler].moves[i];
        if (move != MOVE_NONE && move != MOVE_UNAVAILABLE && ShouldRecordStatusMove(move))
            RecordKnownMove(battler, move);
    }
}
...
uq4_12_t AI_GetMoveEffectiveness(u32 move, u32 battlerAtk, u32 battlerDef)
{
    ...
    uq4_12_t typeEffectiveness = CalcTypeEffectivenessMultiplier(&ctx);
...
uq4_12_t AI_GetTypeEffectivenessAgainstBattler(u32 moveType, u32 battlerDef)
{
    uq4_12_t effectiveness = GetTypeModifier(moveType, gBattleMons[battlerDef].types[0]);

    if (gBattleMons[battlerDef].types[1] != gBattleMons[battlerDef].types[0])
        effectiveness = uq4_12_multiply(effectiveness, GetTypeModifier(moveType, gBattleMons[battlerDef].types[1]));
...
s32 GetBestRecomputedDmgFromBattler(u32 battler, u32 battlerTarget){
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            dmgCalculated = AI_CalcDamage(battlerMove, battler, battlerTarget, &effectiveness, NO_GIMMICK, NO_GIMMICK, weather).median;
...
static bool32 AI_IsSemiInvulnerableAgainstMove(u32 battlerDef, u32 move)
{
    u32 semiInvulnerable = gBattleMons[battlerDef].volatiles.semiInvulnerable;

    if (semiInvulnerable == STATE_PHANTOM_FORCE)
        return TRUE;
    else if (gBattleStruct->battlerState[battlerDef].commandingDondozo)
        return TRUE;
    else if (!MoveDamagesAirborne(move) && semiInvulnerable == STATE_ON_AIR)
        return TRUE;
    else if (!MoveDamagesUnderWater(move) && semiInvulnerable == STATE_UNDERWATER)
        return TRUE;
    else if (!MoveDamagesUnderground(move) && semiInvulnerable == STATE_UNDERGROUND)
        return TRUE;
    else
        return FALSE;
}
...
{
    u32 weather;
    if (AI_IsSemiInvulnerableAgainstMove(battlerDef, move))
        return FALSE;
    ...
    if ((weather & (B_WEATHER_HAIL | B_WEATHER_SNOW)) && MoveAlwaysHitsInHailSnow(move))
        return TRUE;
    if (B_MINIMIZE_DMG_ACC >= GEN_6 && gBattleMons[battlerDef].volatiles.minimize && MoveIncreasesPowerToMinimizedTargets(move))
        return TRUE;
...
{
    if (AI_IsFaster(battlerAtk, battlerDef, gAiThinkingStruct->moveConsidered, GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData), CONSIDER_PRIORITY)
...
bool32 ShouldLowerStat(u32 battlerAtk, u32 battlerDef, u32 defAbility, u32 stat)
{
    switch (stat)
    {
    case STAT_ATK:
        return ShouldLowerAttack(battlerAtk, battlerDef, defAbility);
    case STAT_DEF:
        return ShouldLowerDefense(battlerAtk, battlerDef, defAbility);
    case STAT_SPEED:
        return ShouldLowerSpeed(battlerAtk, battlerDef, defAbility);
    case STAT_SPATK:
        return ShouldLowerSpAtk(battlerAtk, battlerDef, defAbility);
    case STAT_SPDEF:
        return ShouldLowerSpDef(battlerAtk, battlerDef, defAbility);
    case STAT_ACC:
        return ShouldLowerAccuracy(battlerAtk, battlerDef, defAbility);
    case STAT_EVASION:
        return ShouldLowerEvasion(battlerAtk, battlerDef, defAbility);
    default:
        return FALSE;
    }
}
...
bool32 ShouldLowerSpeed(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
    ...
    return (AI_IsSlower(battlerAtk, battlerDef, gAiThinkingStruct->moveConsidered, GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData), CONSIDER_PRIORITY));
}
...
bool32 IsMoveIneffective(u32 move, u32 battlerAtk, u32 battlerDef){
    ...
    if (CanAbilityBlockMove(battlerAtk, battlerDef, gAiLogicData->abilities[battlerAtk], gAiLogicData->abilities[battlerDef], move, AI_CHECK)){
        return TRUE;
    }
    ...
    switch (moveEffect){
        case EFFECT_TORMENT:
            if (moveEffect == EFFECT_TORMENT && gBattleMons[battlerDef].volatiles.torment){
                return TRUE;}
            break;
        case EFFECT_DISABLE:
    ...
    if (move == MOVE_TORMENT && gBattleMons[battlerDef].volatiles.torment){
        return TRUE;
    }
...
bool32 ShouldRecover(u32 battlerAtk, u32 battlerDef, u32 move, u32 healPercent)
{
    ...
    if (move == 0xFFFF || AI_IsFaster(battlerAtk, battlerDef, move, GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData), CONSIDER_PRIORITY))
    {
...
bool32 PartnerMoveHasSameAdditionalEffectSameTarget(u32 battlerAtkPartner, u32 battlerDef, u32 partnerMove, u32 moveEffect, u32 chance)
{
    ...
    for (i = 0; i < GetMoveAdditionalEffectCount(partnerMove); i++)
    {
        ...
        if (additionalEffect->moveEffect == moveEffect && additionalEffect->chance == chance)
            return TRUE;
        if (GetMoveTarget(partnerMove) == MOVE_TARGET_SELECTED)
        {
...
static enum AIScore IncreaseStatUpScoreInternal(u32 battlerAtk, u32 battlerDef, enum StatChange statChange, bool32 considerContrary)
{
    ...
    case STAT_ATK:
        if (HasMoveWithCategory(battlerAtk, DAMAGE_CATEGORY_PHYSICAL) && shouldSetUp)
            tempScore += stages == 1 ? WEAK_EFFECT : DECENT_EFFECT;
        break;
    case STAT_DEF:
    ...
    case STAT_SPEED:
        if ((noOfHitsToFaint >= 3 && !aiIsFaster) || noOfHitsToFaint == UNKNOWN_NO_OF_HITS)
            tempScore += stages == 1 ? DECENT_EFFECT : GOOD_EFFECT;
        break;
    case STAT_SPATK:
...
void IncreaseParalyzeScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    ...
    if (AI_CanParalyze(battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], move, gAiLogicData->partnerMove))
    {
        ...
        bool32 targetIsInfatuated = gBattleMons[battlerDef].volatiles.infatuation;
...
bool32 AI_ShouldTerastal(u32 battler){
    ...
    if (
    AI_GetTypeEffectivenessAgainstBattler(opponentTypes[0], battler) > UQ_4_12(1.0)
    || AI_GetTypeEffectivenessAgainstBattler(opponentTypes[1], battler) > UQ_4_12(1.0)
    ){
        return TRUE;
    }

    //Don't tera if it would go from a STAB immunity to not having a STAB immunity.
    else if (AI_GetTypeEffectivenessAgainstBattler(opponentTypes[0], battler) == UQ_4_12(0)
    && AI_GetTypeEffectivenessAgainstBattler(opponentTypes[1], battler) == UQ_4_12(0)
