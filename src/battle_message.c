...
static const u8 sText_ChallengerIsBatting[]   = _("Challenger is battling {B_TRAINER1_NAME_WITH_CLASS}!\p");
...
const u8 *const gBattleStringsTable[BATTLESTRINGS_COUNT] =
{
    ...
    [STRINGID_STOCKPILEDEFFECTWOREOFF]              = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} consumed Stockpile Energy!"),
    ...
    [STRINGID_DAMP_HEALING]                         = COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX} healed from the moisture!"),
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
        {
            ...
            else
            {
                if (BATTLE_TWO_VS_ONE_OPPONENT)
                    stringPtr = sText_Trainer1WantsToBattle;
                else if (gBattleTypeFlags & BATTLE_TYPE_AI_VS_AI){
                    stringPtr = sText_ChallengerIsBatting;
                }
