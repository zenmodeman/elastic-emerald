...
static u32 GetMoveImmunityValue(enum BattlerId battlerAtk, u32 partyIndex, enum Move move)
{
    ...
    ctx.battlerDef = GetOppositeBattler(battlerAtk);
    ctx.move = move;
    ...
    if (abilityDef != ABILITY_NONE
     && CanAbilityBlockMove(battlerAtk, ctx.battlerDef, abilityAtk, abilityDef, move, AI_CHECK))
        return 2;
    if (CalcPartyMonTypeEffectivenessMultiplier(move, aiMon->species, abilityDef) == UQ_4_12(0.0))
        return 1;
    return 0;
}

static bool32 IsRevealedPlayerMonAvailableForPrediction(enum BattlerId battlerDef, u32 partyIndex)
{
    struct AiPartyMon *aiMon = &gAiPartyData->mons[B_SIDE_PLAYER][partyIndex];
    struct Pokemon *party = GetBattlerParty(battlerDef);

    return aiMon->wasSentInBattle
        && aiMon->species != SPECIES_NONE
        && IsValidForBattle(&party[partyIndex])
        && partyIndex != gBattlerPartyIndexes[battlerDef];
}
...
bool32 TryChooseImmunityPredictionSwitchin(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move)
{
    ...
    for (u32 partyIndex = 0; partyIndex < gAiPartyData->count[B_SIDE_PLAYER]; partyIndex++)
    {
        u32 immunityValue;
        u32 switchCount;

        if (!IsRevealedPlayerMonAvailableForPrediction(battlerDef, partyIndex))
            continue;
        immunityValue = GetMoveImmunityValue(battlerAtk, partyIndex, move);
        if (immunityValue == 0)
            continue;
        switchCount = gAiPartyData->mons[B_SIDE_PLAYER][partyIndex].switchInCount;
...
static bool32 FindMonThatAbsorbsOpponentsMove(enum BattlerId battler)
{
    ...
    if (incomingType == TYPE_WATER || (isOpposingBattlerChargingOrInvulnerable && incomingType == TYPE_WATER))
    {
        ...
        if (GetConfig(REDIRECT_ABILITY_IMMUNITY) >= GEN_5)
    ...
    {
        if (GetConfig(POWDER_OVERCOAT) >= GEN_6)
    ...
    if (gAiThinkingStruct->aiFlags[battler] & AI_FLAG_SMART_SWITCHING)
    {
            ...
            && RandomPercentage(RNG_AI_SWITCH_YAWN, GetSwitchChance(SHOULD_SWITCH_YAWN)))
        {
            ...
            if (gBattleMons[battler].statStages[STAT_EVASION] > (DEFAULT_STAT_STAGE + 3)
                ...
                && (GetConfig(ILLUMINATE_EFFECT) >= GEN_9 && gAiLogicData->abilities[opposingBattler] != ABILITY_ILLUMINATE)
...
static u32 GetSwitchinStatusDamage(enum BattlerId battler)
{
    ...
    if ((status != 0) && ability != ABILITY_MAGIC_GUARD)
    {
        ...
        {
            if (GetConfig(BURN_DAMAGE) >= GEN_7 || GetConfig(BURN_DAMAGE) == GEN_1)
...
static u32 GetSwitchinHitsToKO(s32 damageTaken, enum BattlerId battler, const struct IncomingHealInfo *healInfo, u32 originalHp)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (heldItemEffect == HOLD_EFFECT_FOCUS_SASH || (!opponentCanBreakMold && GetConfig(STURDY) >= GEN_5 && ability == ABILITY_STURDY)) && hitsToKO < 1)
            currentHP = 1;
