...
static u32 GetMoveImmunityValue(enum BattlerId battlerAtk, u32 partyIndex, enum Move move)
{
    ...
    if ((abilityDef == ABILITY_LIGHTNING_ROD && GetConfig(B_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && moveType == TYPE_ELECTRIC)
     || (abilityDef == ABILITY_STORM_DRAIN && GetConfig(B_REDIRECT_ABILITY_IMMUNITY) >= GEN_5 && moveType == TYPE_WATER))
        return 3;
