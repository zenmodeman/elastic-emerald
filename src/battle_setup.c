...
void BattleSetup_StartTrainerBattle(void)
{
    ...
    if (B_FLAG_AI_VS_AI_BATTLE && FlagGet(B_FLAG_AI_VS_AI_BATTLE)){
        ...
        DebugPrintf("AI v.s. AI battle type has been appended.");
    }
    if (InBattlePyramid())
    {
        VarSet(VAR_TEMP_PLAYING_PYRAMID_MUSIC, 0);
...
static void CB2_EndTrainerBattle(void)
{
    ...
    {
        if (InBattlePyramid() || InTrainerHillChallenge() || ShouldContinueAfterLose() || (!NoAliveMonsForPlayer()) || FlagGet(B_FLAG_NO_WHITEOUT))
            SetMainCallback2(CB2_ReturnToFieldContinueScriptPlayMapMusic);
        else
            SetMainCallback2(CB2_WhiteOut);
    }
