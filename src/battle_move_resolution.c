...
static inline bool32 IsSmartTargetMoveConsecutiveHit(enum BattlerId battlerAtk, enum Move move)
{
    ...
    return gMultiHitCounter < GetMoveStrikeCount(move);
}
...
static bool32 TryFormChangeBeforeMove(void)
{
    ...
    bool32 result = TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_BEFORE_MOVE, ability);
    if (!result)
        result = TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_BEFORE_MOVE_CATEGORY, ability);
    if (!result)
        return FALSE;
