...
static enum MoveEndResult MoveEndAbsorb(struct BattleCalcValues *cv)
{
    ...
    if (gBattleMons[cv->battlerAtk].volatiles.drainDouse
    ...
    {
        u32 drainPercent = 33;
        if (IS_BATTLER_OF_TYPE(cv->battlerDef, TYPE_POISON))
            drainPercent = 67;
        else if (IS_BATTLER_OF_TYPE(cv->battlerDef, TYPE_WATER))
            drainPercent = 50;

        s32 drainAmount = gBattleStruct->moveDamage[cv->battlerDef] * drainPercent / 100;
        drainAmount = GetDrainedBigRootHp(cv->battlerAtk, drainAmount);
        if (cv->abilities[cv->battlerDef] == ABILITY_LIQUID_OOZE)
            drainAmount = -drainAmount;

        // Drain Douse is additional to a move's native draining effect.
        if (cv->moveEffect == EFFECT_ABSORB || cv->moveEffect == EFFECT_DREAM_EATER)
        {
            s32 nativeDrain = gBattleStruct->moveDamage[cv->battlerDef] * GetMoveAbsorbPercentage(cv->move) / 100;
            nativeDrain = GetDrainedBigRootHp(cv->battlerAtk, nativeDrain);
            if ((cv->moveEffect == EFFECT_DREAM_EATER && GetConfig(B_DREAM_EATER_LIQUID_OOZE) < GEN_5)
             || cv->abilities[cv->battlerDef] != ABILITY_LIQUID_OOZE)
        ...
        if (drainAmount > 0)
        {
            SetHealAmount(cv->battlerAtk, drainAmount);
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_DRAIN_DOUSED;
            BattleScriptCall(BattleScript_EffectAbsorb);
        }
        else if (drainAmount < 0)
        {
            SetPassiveDamageAmount(cv->battlerAtk, -drainAmount);
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_ABSORB_OOZE;
            BattleScriptCall(BattleScript_EffectAbsorbLiquidOoze);
        }
        gBattleScripting.moveendState++;
...
static enum MoveEndResult MoveEndAbilitiesAttacker(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_MOVE_END_ATTACKER, cv->battlerAtk, 0, 0, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
    else if (cv->abilities[cv->battlerAtk] == ABILITY_ILLUMINATE
          && IsIlluminatingMove(cv->move)
          && IsDoubleBattle()
          && gSideTimers[GetBattlerSide(cv->battlerAtk)].followmeTimer == 0)
    {
        gSideTimers[GetBattlerSide(cv->battlerAtk)].followmeTimer = 1;
        gSideTimers[GetBattlerSide(cv->battlerAtk)].followmeTarget = cv->battlerAtk;
        gBattleScripting.battler = gBattlerAbility = cv->battlerAtk;
        BattleScriptCall(BattleScript_IlluminateActivates);
        result = MOVEEND_RESULT_RUN_SCRIPT;
    }
    else if (cv->abilities[cv->battlerAtk] == ABILITY_MERRY
          && IsGiftingMove(cv->move)
          && !gBattleMons[cv->battlerAtk].volatiles.merry)
    {
        gBattleMons[cv->battlerAtk].volatiles.merry = TRUE;
        gBattleScripting.battler = gBattlerAbility = cv->battlerAtk;
        BattleScriptCall(BattleScript_MerryActivates);
        result = MOVEEND_RESULT_RUN_SCRIPT;
    }
