...
static bool32 UpdateDefensiveContactAbilityCounter(u32 battler, bool32 triggered);
static bool32 TryDefensiveContactAbilityTrigger(u32 battler, enum RandomTag tag);
static u32 GetEffectSporeMoveEffect(u32 preferredEffect, bool32 canPoison, bool32 canParalyze, bool32 canSleep);
...
static u32 GetEffectSporeMoveEffect(u32 preferredEffect, bool32 canPoison, bool32 canParalyze, bool32 canSleep)
{
    ...
    if (preferredEffect == MOVE_EFFECT_PARALYSIS && canParalyze)
        return MOVE_EFFECT_PARALYSIS;
    if (preferredEffect == MOVE_EFFECT_SLEEP && canSleep)
        return MOVE_EFFECT_SLEEP;

    if (canPoison && preferredEffect != MOVE_EFFECT_POISON)
        effects[numEffects++] = MOVE_EFFECT_POISON;
    if (canParalyze && preferredEffect != MOVE_EFFECT_PARALYSIS)
        effects[numEffects++] = MOVE_EFFECT_PARALYSIS;
    if (canSleep && preferredEffect != MOVE_EFFECT_SLEEP)
        effects[numEffects++] = MOVE_EFFECT_SLEEP;
    ...
    case 1:
        return effects[0];
    case 2:
        return effects[RandomUniform(RNG_EFFECT_SPORE_STATUS, 0, 1)];
    default:
        return RandomElement(RNG_EFFECT_SPORE_STATUS, effects);
    }
}
        ...
        case ABILITY_ANTICIPATION:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                ...
                if (effect != 0)
                {
                    ...
                    gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                    gDisableStructs[battler].anticipationActivated = TRUE;
                    BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                }
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_ASTRAL_CHARGE:
            if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT)
             ...
             && IsBattlerAlive(battler)
             && (moveType == TYPE_FAIRY || moveType == TYPE_PSYCHIC)
        ...
        case ABILITY_EFFECT_SPORE:
        {
            ...
            {
                u32 poison, paralysis, sleep, preferredEffect;
                ...
                if (UpdateDefensiveContactAbilityCounter(gBattlerTarget, i < sleep))
                {
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    ...
    case MOVE_EFFECT_PARALYSIS:
        ...
        else if (abilityDef == ABILITY_LIMBER)
        {
            abilityAffected = TRUE;
            battleScript = BattleScript_ImmunityProtected;
        }else if (gFieldStatuses & STATUS_FIELD_MUDSPORT &&  GetBattleMoveType(gCurrentMove) == TYPE_ELECTRIC){
            battleScript = BattleScript_MudSportPrevents;
        }
        break;
    case MOVE_EFFECT_BURN:
        ...
        }else if (gFieldStatuses & STATUS_FIELD_WATERSPORT &&  GetBattleMoveType(gCurrentMove) == TYPE_FIRE){
            battleScript = BattleScript_WaterSportPrevents;
        }
        break;
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        else if(abilityDef == ABILITY_LEAF_GUARD && !IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM)
            && !IsBattlerWeatherAffected(battlerDef, B_WEATHER_HAIL) && !IsBattlerWeatherAffected(battlerDef, B_WEATHER_RAIN)){
                abilityAffected = TRUE;
                battleScript = BattleScript_NotAffected;
            }
        break;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_RIVALRY:
        if (AreBattlersOfSameGender(battlerAtk, battlerDef))
            //Increased form 1.25 to 1.33 to achieve parity
...
static inline uq4_12_t GetOtherModifiers(struct DamageContext *ctx)
{
    ...
    if (ctx->move == MOVE_BOUNCE && (gBattleMons[ctx->battlerAtk].species == SPECIES_SPOINK || gBattleMons[ctx->battlerAtk].species == SPECIES_GRUMPIG))
...
void TryRestoreHeldItems(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if ((B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9 && !FlagGet(FLAG_RESOURCE_MODE)) || gBattleStruct->itemLost[B_SIDE_PLAYER][i].stolen || returnNPCItems)
        {
            ...
            if ((lostItem != ITEM_NONE || returnNPCItems))
                SetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM, &lostItem); // Restore all items, including berries
        }
    }
}
