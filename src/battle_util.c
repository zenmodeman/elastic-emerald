...
bool32 CanBeFrozen(u32 battler)
{
    ...
    if (IS_BATTLER_OF_TYPE(battler, TYPE_ICE)
     ...
     || (ability == ABILITY_LEAF_GUARD && !IsBattlerWeatherAffected(battler, B_WEATHER_SANDSTORM) 
     && !IsBattlerWeatherAffected(battler, B_WEATHER_HAIL) && !IsBattlerWeatherAffected(battler, B_WEATHER_RAIN))
     || ability == ABILITY_COMATOSE
     || ability == ABILITY_PURIFYING_SALT
     || gBattleMons[battler].status1 & STATUS1_ANY
     || IsAbilityStatusProtected(battler)
...
static u8 HealConfuseBerry(u32 battler, u32 itemId, u32 flavorId, enum ItemEffect caseID)
{
     ...
     && (B_HEAL_BLOCKING < GEN_5 || !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)))
    {
        ...
        gBattleStruct->moveDamage[battler] = (GetNonDynamaxMaxHP(battler) * 40 / 100);
...
static u8 ItemHealHp(u32 battler, u32 itemId, bool32 caseID, bool32 percentHeal)
{
    ...
    case ITEMEFFECT_LIFEORB_SHELLBELL:
        ...
        case HOLD_EFFECT_LIFE_ORB:
                ...
                && gSpecialStatuses[gBattlerAttacker].damagedMons)
            {
                ...
                RecordItemEffectBattle(gBattlerAttacker, HOLD_EFFECT_LIFE_ORB);
            }
            break;
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    if (damageCalcData->isCrit && defStage > DEFAULT_STAT_STAGE && !(defAbility == ABILITY_BIG_PECKS && IsBattleMovePhysical(move)))
    ...
    if (defAbility != ABILITY_BIG_PECKS && (MoveIgnoresDefenseEvasionStages(move) || gMovesInfo[move].effect == EFFECT_PUNISHMENT))
        defStage = DEFAULT_STAT_STAGE;
...
static inline uq4_12_t GetScreensModifier(u32 move, u32 battlerAtk, u32 battlerDef, bool32 isCrit, u32 abilityAtk)
{
    ...
    if (!(GetBattlerAbility(battlerDef) == ABILITY_BIG_PECKS &&  (reflect || (auroraVeil && IsBattleMovePhysical(move))))){
...
{
    //Note that this code now deivates from expansion which uses B_RETURN_STOLEN_NPC_ITEMS
    u32 i;
    u16 lostItem = ITEM_NONE;

    for (i = 0; i < PARTY_SIZE; i++)
    {
        if ((B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9 && !FlagGet(FLAG_RESOURCE_MODE)) || gBattleStruct->itemLost[B_SIDE_PLAYER][i].stolen)
        {
            ...
            //In a trainer battle, ITEM_NONE is restored; i.e., a tricked item is not persisted
            if (lostItem != ITEM_NONE || gBattleTypeFlags & BATTLE_TYPE_TRAINER) 
            SetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM, &lostItem);  // Restore all items
