    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (moves[i] != MOVE_NONE && GetMovePower(moves[i]) != 0)
        {
            ...
            if (ShouldConsiderWrapDamageCombo(battlerAtk, battlerDef, moves[i])){
                ...
                if (!AI_IsFaster(battlerAtk, battlerDef, moves[i]) && numRemainingActionsBeforeKOd != 0)
...
case EFFECT_DISABLE:
    ...
    case EFFECT_DEFENSE_CURL:
        ...
        if (HasMoveWithEffect(battlerAtk, EFFECT_ROLLOUT) && !(gBattleMons[battlerAtk].status2 & STATUS2_DEFENSE_CURL) && !(CanTargetFaintAiWithMod(battlerDef, battlerAtk, 0, 2)))
            {
