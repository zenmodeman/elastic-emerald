    ...
    while (flags != 0)
    {
        if (flags & 1)
        {
            if (IsBattlerPredictedToSwitch(gBattlerTarget) && (AI_THINKING_STRUCT->aiFlags[battlerAi] & AI_FLAG_PREDICT_INCOMING_MON)){
                DebugPrintf("Predicted Switch in processing with switchInUser: %d",AI_DATA->mostSuitableMonId[gBattlerTarget]);
...
// the following checks apply to any target (including user)
    ...
    // stat lowering effects
        ...
        case EFFECT_NOBLE_ROAR:
            ...
            && !ShouldLowerStat(battlerAtk, battlerDef, aiData->abilities[battlerDef], STAT_ATK)){
                ADJUST_SCORE(-10);
            }
            break;
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_SPECIAL_DEFENSE_UP_2:
        ...
        if (HasMoveEffect(battlerDef, EFFECT_SPECIAL_DEFENSE_DOWN_2) && AI_RandLessThan(127)){
            ADJUST_SCORE(-1);
            break;
        }
    ...
    case EFFECT_LEECH_SEED:
        ...
        if (!CanAIFaintTarget(battlerAtk, battlerDef, 2)){
            if (IsBattlerTrapped(battlerDef, FALSE) && aiData->holdEffects[battlerAtk] == HOLD_EFFECT_BIG_ROOT && AI_RandLessThan(250)){
                ADJUST_SCORE(DECENT_EFFECT);
            }else if ((IsBattlerTrapped(battlerDef, FALSE) || aiData->holdEffects[battlerAtk] == HOLD_EFFECT_BIG_ROOT) && AI_RandLessThan(200)){
                ADJUST_SCORE(WEAK_EFFECT);
            }
...
case EFFECT_DISABLE:
    ...
    case EFFECT_DEFOG:
        ...
        break;
    case EFFECT_TORMENT:
        //Don't go for torment if can make enough progress
        if (!CanAIFaintTarget(battlerAtk, battlerDef, 2)){
            //If AI does take a lot of damage from the matchup, at least still allow it if there are mons in the back
            if (!CanTargetFaintAiWithMod(battlerDef, battlerAtk, 0, 2) || CountUsablePartyMons(battlerAtk) > 0){
                ADJUST_SCORE(WEAK_EFFECT);
            }
        }
        break;
    ...
    case EFFECT_SOAK:
        ...
        if (HasMoveWithType(battlerAtk, TYPE_ELECTRIC) || HasMoveWithType(battlerAtk, TYPE_GRASS) || 
            (HasMoveEffect(battlerAtk, EFFECT_SUPER_EFFECTIVE_ON_ARG) && GetMoveArgType(move) == TYPE_WATER)){
                ...
                case MOVE_EFFECT_KNOCK_OFF:
                    if (CanKnockOffItem(battlerDef, aiData->items[battlerDef]))
                    {
                        ...
                        case HOLD_EFFECT_STICKY_BARB:
                            break;
                        default:
                            //Make it so the chance to be incentivized beyond best damage is occasional
                            if (AI_RandLessThan(127)){
                                ADJUST_SCORE(WEAK_EFFECT);
                            }
                            break;
