...
void CustomTrainerPartyAssignMoves(struct Pokemon *mon, const struct TrainerMon *partyEntry)
{
    ...
    for (j = 0; j < MAX_MON_MOVES; ++j)
    {
        u32 pp = CalculatePPWithBonus(partyEntry->moves[j], partyEntry->ppBonuses, j);
...
static void ClearSetBScriptingStruct(void)
{
    ...
    #endif
    gBattleScripting.expOnCatch = (GetConfig(EXP_CATCH) >= GEN_6);
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!IsBattleControllerActiveOrPendingSyncAnywhere(battler))
            {
                ...
                case B_ACTION_USE_MOVE:
                    ...
                    else if (GetConfig(ENCORE_TARGET) < GEN_5 && gBattleMons[battler].volatiles.encoredMove != MOVE_NONE)
                    {
...
u32 GetBattlerTotalSpeedStat(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (gBattleMons[battler].status1 & STATUS1_PARALYSIS && ability != ABILITY_QUICK_FEET)
        speed /= GetConfig(PARALYSIS_SPEED) >= GEN_7 ? 2 : 4;
...
s32 GetBattleMovePriority(enum BattlerId battler, enum Ability ability, enum Move move)
{
    ...
    if (gProtectStructs[battler].quash)
    {
        priority = -8;
          ...
          && (GetConfig(GALE_WINGS) < GEN_7 || IsBattlerAtMaxHp(battler))
...
static bool32 TryDoGimmicksBeforeMoves(void)
{
    ...
    if (GetConfig(MEGA_EVO_TURN_ORDER) >= GEN_7)
