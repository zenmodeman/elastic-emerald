...
static const u8 sLandEncounterCumulativeChances[LAND_WILD_COUNT] =
{
...
u32 ChooseWildMonIndex_Land(void)
{
    ...
    while (wildMonIndex < LAND_WILD_COUNT - 1 && rand >= sLandEncounterCumulativeChances[wildMonIndex])
        wildMonIndex++;
    ...
    if (swap)
        wildMonIndex = LAND_WILD_COUNT - 1 - wildMonIndex;
...
// SHAKE_WILD_COUNT
static u32 ChooseWildMonIndex_Shake(void)
{
    u32 wildMonIndex = 0;
    bool8 swap = FALSE;
    u8 rand = Random() % ENCOUNTER_CHANCE_SHAKE_MONS_TOTAL;

    if (rand < ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_0)
        wildMonIndex = 0;
    else if (rand >= ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_0 && rand < ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_1)
        wildMonIndex = 1;
    else if (rand >= ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_1 && rand < ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_2)
        wildMonIndex = 2;
    else if (rand >= ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_2 && rand < ENCOUNTER_CHANCE_SHAKE_MONS_SLOT_3)
        wildMonIndex = 3;
    else
        wildMonIndex = 4;
...
enum TimeOfDay GetTimeOfDayForEncounters(u32 headerId, enum WildPokemonArea area)
{
    ...
    case WILD_AREA_SHAKE:
        wildMonInfo = gWildMonHeaders[headerId].encounterTypes[timeOfDay].shakeMonsInfo;
        break;
    }
...
bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo, enum WildPokemonArea area, u8 flags)
{
    ...
    if (area == WILD_AREA_LAND && monotype == TYPE_NONE && wildMonIndex >= MAX_REGULAR_LAND_SLOTS)
        wildMonIndex %= MAX_REGULAR_LAND_SLOTS;
...
void ShakeWildEncounter(void)
{
    ...
    if (headerId != HEADER_NONE)
    {
        enum TimeOfDay timeOfDay = GetTimeOfDayForEncounters(headerId, WILD_AREA_SHAKE);
        const struct WildPokemonInfo *wildPokemonInfo = gWildMonHeaders[headerId].encounterTypes[timeOfDay].shakeMonsInfo;

        if (wildPokemonInfo == NULL || GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES_OR_EGG) == SPECIES_NONE)
        {
            gSpecialVar_Result = FALSE;
        }
        else if (WildEncounterCheck(wildPokemonInfo->encounterRate, TRUE) == TRUE
              && TryGenerateWildMon(wildPokemonInfo, WILD_AREA_SHAKE, WILD_CHECK_REPEL | WILD_CHECK_KEEN_EYE) == TRUE)
        {
            BattleSetup_StartWildBattle();
            gSpecialVar_Result = TRUE;
        }
