...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    else if (evasionStage > DEFAULT_STAT_STAGE && atkAbility == ABILITY_LONG_REACH)
        evasionStage = DEFAULT_STAT_STAGE;
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_PAYDAY:
                ...
                //Make Pay Day work for both players
            ...
            case MOVE_EFFECT_HAPPY_HOUR:
                //Make Happy Hour work for either player
            ...
            case MOVE_EFFECT_PSYCHIC_TERRAIN:
            {
                ...
                if (!(gFieldStatuses & statusFlag) && statusFlag != 0)
                {
                    ...
                    if ((GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_TERRAIN_EXTENDER)
                        || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED)
            ...
            case MOVE_EFFECT_GRAVITY:
                if (!(gFieldStatuses & STATUS_FIELD_GRAVITY))
                {
                    ...
                    gFieldTimers.gravityTimer = gBattleTurnCounter + 5 + (GetBattlerAbility(gBattlerAbility) == ABILITY_DEDICATED ? 3 : 0);
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (FlagGet(FLAG_LEVEL_CAP) && gBattleStruct->battlerExpReward != 0)
                    {
    ...
    do
    {
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            if (!IsOnPlayerSide(gBattlerAttacker))
                UpdateStallMons();
            //Added MOVE_RESULT_MISSED here since it seems to make sense for Stomping
            //Though from testing it appears that missed moves were already being computed
            //Mainly adding this because Truant also uses MOVE_RESULT_MISSED 
            if ((gBattleStruct->moveResultFlags[gBattlerTarget] & (MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_MISSED))
...
static void Cmd_setmist(void)
{
    ...
    else
    {
        ...
        gSideTimers[GetBattlerSide(gBattlerAttacker)].mistTimer = (atkHoldEffect == HOLD_EFFECT_ICY_ROCK || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED) 
        ? gBattleTurnCounter + 8 : gBattleTurnCounter + 5;
...
static void Cmd_curestatuswithmove(void)
{
    ...
    if (GetMoveEffect(gCurrentMove) == EFFECT_REFRESH)
        shouldHeal = gBattleMons[gBattlerAttacker].status1 
        // & STATUS1_REFRESH;
        & STATUS1_ANY; //Altering Refresh to work for any status
    else // Take Heart
...
static void HandleRoomMove(u32 statusFlag, u16 *timer, u8 stringId)
{
    ...
    {
        gFieldStatuses |= statusFlag;
        *timer = gBattleTurnCounter + 5 + ((GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED) ? 3 : 0);
    ...
    if (GetMoveEffect(gCurrentMove) == EFFECT_MUD_SPORT)
    {
        if (B_SPORT_TURNS >= GEN_6)
        {
            if (!(gFieldStatuses & STATUS_FIELD_MUDSPORT))
            {
                ...
                gFieldTimers.mudSportTimer =  (atkHoldEffect == HOLD_EFFECT_SMOOTH_ROCK || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED)
                ? gBattleTurnCounter + 8 : gBattleTurnCounter + 5;
    ...
    else // Water Sport
    {
        if (B_SPORT_TURNS >= GEN_6)
        {
            if (!(gFieldStatuses & STATUS_FIELD_WATERSPORT))
            {
                ...
                gFieldTimers.waterSportTimer = (atkHoldEffect == HOLD_EFFECT_DAMP_ROCK || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED)
                 ? gBattleTurnCounter + 8 : gBattleTurnCounter + 5;
...
u32 GetTerrainType(){
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        ...
        return TYPE_NORMAL; // Default/failsafe
}
...
void BS_SetTerrain(void)
{
    ...
    else
    {
        gFieldStatuses &= ~STATUS_FIELD_TERRAIN_ANY;
        gFieldStatuses |= statusFlag;
        gFieldTimers.terrainTimer = gBattleTurnCounter
            + ((atkHoldEffect == HOLD_EFFECT_TERRAIN_EXTENDER || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEDICATED) ? 8 : 5);
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
