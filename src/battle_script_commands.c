...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            ...
            break;
        case MOVEEND_ABSORB:
            if ((moveEffect == EFFECT_ABSORB || gBattleMons[gBattlerAttacker].volatiles.drainDouse)
             && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
...
static void Cmd_trysetvolatile(void)
{
    ...
    if (cmd->_volatile != VOLATILE_LASER_FOCUS && GetBattlerVolatile(battler, cmd->_volatile))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
...
void BS_ApplySaltCure(void)
{
    ...
    u8 battler = GetBattlerForBattleScript(cmd->battler);
    gBattleMons[battler].volatiles.saltCure = TRUE;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
